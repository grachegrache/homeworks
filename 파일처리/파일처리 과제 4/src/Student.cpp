...
int Student::Pack(IOBuffer& buffer){
	int result;
	buffer.Clear();
	result = buffer.Pack((const char*) id);
	result = result && buffer.Pack((const char*) name);
	result = result && buffer.Pack((const char*) address);
	result = result && buffer.Pack((const char*) enrollment);
	result = result && buffer.Pack((const char*) credit_hours);
	return result;
}
...
int Student::InitBuffer(FixedTextBuffer& buffer){
	...
	result = result && buffer.AddField(10);  // credit hours
	return result;
}
