...
//	and to fetch the values of fields (Unpack)
{
	public:
		...
		int Pack(const char* field, int size = -1); // set the value of the next field of the buffer;
		int Unpack(char* field, int maxBytes = -1); // extract the value of the next field of the buffer
