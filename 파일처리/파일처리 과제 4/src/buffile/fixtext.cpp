...
int FixedTextBuffer::Unpack(char * str, int maxBytes)
...
char* FixedTextBuffer::getBuffer(){
	return Buffer;
}

int FixedTextBuffer::isEqualField(int fieldNum, const char* field){
	int i, offset;
	offset = 0;

	for(i = 0; i < fieldNum; i++)
		offset += FieldSize[i];
	char found_field[FieldSize[fieldNum]];
	strncpy(found_field, &Buffer[offset], FieldSize[fieldNum]);
	int result = strcmp(found_field, field);

	return result;
}

void FixedTextBuffer::setPacking(int _bool){
	Packing = _bool;
}
